...
## ⚠️ 待处理
- 按 s01 → s20 顺序逐章填写章节笔记
- 每章至少补齐：核心收获、源码阅读摘要、一个流程图或状态图、一个实验记录
- 学到稳定概念后，拆出 `concepts/` 下的概念卡片

## 🔗 活跃线索
- agent loop → tool use → permission/hooks → task/context/memory → agent teams/worktree/MCP
- Git worktree / rebase → AI agent 隔离开发 → 更安全的编码工作流
- claude-obsidian 插件 → 个人知识管理工作流自动化

## 🧠 关键上下文
- Vault 是 Obsidian 知识库，正式新内容优先进入 `wiki/`
- 用户是 Java 后端转 AI 应用开发的工程师
- 工作领域：医保核赔系统设计与开发
- 笔记风格：长文深度笔记、Key Takeaways 前置、Mermaid 图表、富表格
