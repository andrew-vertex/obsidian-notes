---
title: Tech 领域索引
created: 2026-05-07
description: Tech 领域知识库入口，负责把问题路由到 Architecture、Backend、DevOps 等子域。
tags:
  - index
  - tech
  - knowledge-base
---
# Tech 领域索引

`Tech/` 下的现有主题目录就是正式 wiki 层，`Tech/_kb/` 只负责 source、schema、index 和 log。

[TOC]

## Subtopics

- `AI/`: AI 应用开发学习体系，包括大模型基础、Prompt/Context Engineering、RAG、MCP、评测、模型网关与 AI 系统设计
- `Agent/`: agent 使用、扩展、工作流与跨代理协作
- `Algorithms/`: 算法与题解类知识
- `Architecture/`: 架构原则、分布式设计、性能与系统设计
- `Backend/`: 数据库、搜索引擎、微服务与后端主题
- `DevOps/`: CI/CD、Docker、Kubernetes、Linux
- `Programming/`: 语言与通用编程主题

## Canonical Notes

- [[../AI/Hello-Agents/index|Hello-Agents 学习索引]]: Datawhale《从零开始构建智能体》的个人课程地图、学习进度和章节笔记入口。
- [[../AI/Hello-Agents/第一部分：智能体与语言模型基础/第一章 初识智能体]]: 从经典 AI 与 LLM 工程视角理解 Agent 定义、分类、PEAS、运行循环、协作模式和生产边界。
- [[../AI/Java 转 AI 应用开发/index|Java 转 AI 应用开发索引]]: Java 后端转向 AI 应用工程的主入口，包含 8 阶段路线、专题阅读骨架与实践验收清单。
- [[../AI/AI 应用开发学习体系]]: JavaGuide AI 专题的旧版粗粒度主题总览，已回链到新版学习工作区。
- [[../Agent/AI Agent 的 Harness Engineering]]: AI Agent 从 demo 走向生产力时的控制面方法论，包括真相源、执行边界、能力路由与验证闭环。
- [[../Backend/DateBase/elasticsearch/Elasticsearch 阅读指南]]: Elasticsearch 主题第一入口，先给出阅读顺序，再路由到基础、查询、分词、原理、分页、集群与数据同步页面。
