...
1. 在合适的 `_kb/raw/` 下创建一个中文名的来源记录，例如 `<主题> 来源记录.md`
...
```text
Use $llm-wiki-curator to ingest `Clippings/<file>.md`, decide the right knowledge domain, create a Chinese-named source record under that domain’s `_kb/raw/`, create or update the Chinese-named canonical note in the domain note tree, and refresh the domain index and log.
```
